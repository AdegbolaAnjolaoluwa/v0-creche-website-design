...
export default clerkMiddleware(async (auth, req) => {
  ...
  // Relax CSP slightly for Clerk dev environments which might use different subdomains
  ...
  if (isProtected(req)) {
      // Check if we are in dev mode or using test keys in prod, which might cause issues if keys are mismatched
      try {
        ...
        // If auth() fails (e.g. key mismatch), redirect to home instead of crashing
